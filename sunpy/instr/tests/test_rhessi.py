...
def cross_month_timerange():
    ...
    Dbase files are monthly therefore this is to make sure that two dbase files are returned.
    """
...
def test_parse_observing_summary_dbase_file():
    """
    Test that we get the observing summary dbase file with the content we expect.
    """
...
def test_parse_observing_summary_dbase_file_mock():
    """
    Ensure that all required data are extracted from the RHESSI observing summary database file
    mocked in `hessi_data()`
    """
