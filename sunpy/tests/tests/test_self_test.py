...
def test_import_runner():
    """
    When running the tests with setup.py, the test runner class is imported by setup.py before
    coverage is watching.

    To ensure that the coverage for sunpy/tests/runner.py is correctly measured we force the
    interpreter to reload it here while coverage is watching.
    """
...
def test_main_stdlib_module():
    """
    This test makes sure that the module is really searched within the sunpy package.
    """
    with pytest.raises(ValueError):
