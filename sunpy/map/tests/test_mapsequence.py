...
def masked_aia_map(aia_map):
    ...
    A rectangular (not square) block of True values are included to test that operations on the mask
    respect how the mask is stored.
    """
...
def mapsequence_all_the_same_all_have_masks(masked_aia_map):
    """
    Simple `sunpy.map.mapsequence` for testing, in which all the maps have masks.
    """
...
def mapsequence_all_the_same_some_have_masks(aia_map, masked_aia_map):
    """
    Simple `sunpy.map.mapsequence` for testing, in which at least some of the maps have masks.
    """
...
def mapsequence_different(aia_map):
    """
    Mapsequence allows that the size of the image data in each map be different.
...
def test_as_array(mapsequence_all_the_same,
                  ...
                  mapsequence_all_the_same_some_have_masks):
    """
    Make sure the data in the mapsequence returns correctly, when all the maps have the same shape.
...
def test_all_meta(mapsequence_all_the_same):
    """
    Tests that the correct number of map meta objects are returned, and that they are all map meta
    objects.
    """
