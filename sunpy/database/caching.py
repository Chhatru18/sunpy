"""
This module implements caching support for the database package.
"""
...
class BaseCache(object, metaclass=ABCMeta):
    ...
    It has a certain capacity, stored in the attribute `maxsize`. Whether this capacity is reached,
    can be checked by using the boolean property `is_full`. To implement a custom cache, inherit
    from this class and override the methods ``__getitem__`` and ``__setitem__``. Call the method
    `sunpy.database.caching.BaseCache.callback` as soon as an item from the cache is removed.
    """
    ...
    def get(self, key, default=None):  # pragma: no cover
        ...
        This method has no side-effects, multiple calls with the same cache and the same passed key
    ...
    def __getitem__(self, key):
        """
        abstract method: this method must be overwritten by inheriting subclasses.

        It defines what happens if an item from the cache is attempted to be accessed.
        """
    ...
    def __setitem__(self, key, value):
        ...
        It defines what happens if a new value should be assigned to the given key. If the given key
        does already exist in the cache or not must be checked by the person who implements this
        method.
        """
    ...
    def remove(self):
        ...
        Which item is removed, depends on the implementation of the cache. After the item has been
        removed, the callback method is called.
        """

    def callback(self, key, value):
        """
        This method should be called (by convention) if an item is removed from the cache because it
        is full.

        The passed key and value are the ones that are removed. By default this method does nothing,
        but it can be customized in a custom cache that inherits from this base class.
        """

    @property
    def is_full(self):
        """
        True if the number of items in the cache equals :attr:`maxsize`, False otherwise.
        """
...
class LRUCache(BaseCache):
    ...
    def __getitem__(self, key):
        """
        Returns the value which is associated to the given key and put it with its associated value
        to the end of this cache.
    ...
    def __setitem__(self, key, value):
        """
        If the key does already exist in the cache, move it to the end of this cache.

        Otherwise, set a new value and put it to the end of this cache. If the cache is full, remove
        the least recently used item before inserting the new key-value pair.
        """
        if key in self:
            del self[key]
        if self.is_full:
...
class LFUCache(BaseCache):
    ...
    def __getitem__(self, key):
        """
        Returns the value which is associated to the given key and increments the frequency counter
        of this key.
    ...
    def __setitem__(self, key, value):
        """
        Increment the frequency counter of the given key if it is already present in the cache,
        otherwise set it to 1.

        If the cache is full, remove the least frequently used item before inserting the new key-
        value pair.
        """
        self.usage_counter[key] += 1
        if self.is_full:
